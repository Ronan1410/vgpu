//represents the computer, the combination of processor (proc) and the memory controller (mem)
module comp(
    input rst, clk,
    input [15:0] oob_write_addr,
    input [15:0] oob_write_data,
    input oob_write_wen,

    output reg [3:0] op,
    output reg [3:0] reg_select,
    output reg [7:0] p1,
    output reg [7:0] x1,
    output reg [15:0] pc,
);

    wire mem_we;

    reg [15:0] mem_read_addr, mem_write_addr;
    reg [15:0] mem_write_data, mem_read_data;

    mem mem1(
        .clk(clk), we(mem_we),
        .read_addr(mem_read_addr), .write_addr(mem_write_addr),
        .read_data(mem_read_data), .write_data(mem_write_data),
        .oob_write_addr(oob_write_addr), .oob_write_data(oob_write_data),
        .oob_write_wen(oob_write_wen)
    );

    proc proc1(
        .rst(rst), .clk(clk),
        .out(out), .op(op), .p1(p1), .pc(pc),
        .reg_select(reg_select), .x1(x1),
        .mem_read_addr(mem_read_addr), .mem_write_addr(mem_write_addr),
        .mem_read_data(mem_read_data), .mem_write_data(mem_write_data),
        .mem_we(mem_we)
    )
endmodule